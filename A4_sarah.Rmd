...
described <- df %>% split(df$file) %>% map(summary) 
...
# plot data for one group at a time?
  # and what about respiration? do the same?
x1 <- x$`11`
x1 <- x$`48`
x1 <- x$`41`
...
k <- ggplot(data = x1) + 
  ...
  labs(x = "time", y = "HR") + facet_grid(~trial, scales = 'free')
...
data_test <- data_test %>% 
  replace_with_na_at(.vars = c("HR1"),
                     condition = ~.x %in% HR1)

sum(is.na(data_test$HR2))
...
Resp1 <- as.list(df_Resp1$Resp1)
Resp1 <- unique(Resp1)

data_test <- data_test %>% 
  replace_with_na_at(.vars = c("Resp1"),
                     condition = ~.x %in% Resp1)
...
#then go from wide to long

#then make sure that the values are checkered

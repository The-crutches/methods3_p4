...
```{r}
# Load the libraries
pacman::p_load(tidyverse, tidymodels, fs, groupdata2, dplyr, purrr, naniar)
...
HR1_remove <- unique(as.list(as.complex(df_HR1$HR1)))
...
HR2_remove <- unique(as.list(as.complex(df_HR2$HR2)))
...
Resp1_remove <- unique(as.list(as.complex(df_Resp1$Resp1)))
...
Resp2_remove <- unique(as.list(as.complex(df_resp2$Resp2)))

data <- data %>% 
  replace_with_na_at(.vars = c("Resp2"),
                     condition = ~.x %in% Resp2_remove)

sum(is.na(data$Resp1))

n_occur_3 <- data.frame(table(data$Resp1))
df_Resp1 <- data[data$Resp1 %in% n_occur_3$Var1[n_occur_3$Freq > 5],]

Resp1 <- as.list(as.complex(df_Resp1$Resp1))
Resp1 <- unique(Resp1)

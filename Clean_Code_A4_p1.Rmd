...
```{r}
Groups <- as.numeric(as.character(unique(df$group[df$study == 1])))

SurrogateList <- expand.grid(a = Groups, b = Groups) %>% filter(a!=b)

Groups2 <- as.numeric(as.character(unique(df$group[data$study == 2])))

SurrogateList2 <- expand.grid(a = Groups2, b = Groups2) %>% filter(a!=b)

Groups3 <- as.numeric(as.character(unique(data$group[data$study == 3])))

SurrogateList3 <- expand.grid(a = Groups3, b = Groups3) %>% filter(a!=b)

Groups4 <- as.numeric(as.character(unique(data$group[data$study == 4])))

SurrogateList4 <- expand.grid(a = Groups4, b = Groups) %>% filter(a!=b)

SurrogateList_a <- rbind(SurrogateList1, SurrogateList12, SurrogateList13)
SurrogateList_b <- rbind(SurrogateList, SurrogateList2, SurrogateList3, SurrogateList4)

for (i in 1:row(SurrogateList_a)){
  ...
  for(co in c("Synchronous", "TurnTaking", "SelfPaced", "Conversation")) {
    if(co %>% unique(x$type) & co %>% unique(y$type)){
      z1 <- subset(x, type == co)
      z2 <- subset(y, type == co)
    }
    if(nrow(z1) > nrow(z2)){
      z1 <- z1[-((nrow(z1)+1):nrow(z2)),]
    }
    if(nrow(z2) > nrow(z1)){

...
set.seed(125)
sur <- df1 %>% 
  #Grouping by study and trial
  group_by(study,trial)
  
#Getting unique participant IDs
sur <- summarise(ID == unique(ID))

#Extracting info on whether they are par1 or par2 in a pair
sur <- sur %>% 
  mutate(par = str_extract(as.character(ID), "(\d)[^\d]*$"), sur)

#Making seperate columns for par1 and par2 
pivot_wider(names_from = participant, values_from = participant, data = sur) 

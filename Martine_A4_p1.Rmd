...
# Load the file
data <- read_csv("Study1_G1_T1_Synchronous.csv")
# Plot
## Remove outliers
...
data1 <- data %>%
  ...
  #Do we want to use another method?
  dplyr::summarise(
    time = mean(time,na.rm=T),
    HR1_c = mean(HR1_c,na.rm=T),
    HR2_c = mean(HR2_c,na.rm=T),
    Resp1_c = mean(Resp1_c,na.rm=T),
    Resp2_c = mean(Resp2_c,na.rm=T))
...
id_stuff <- str_match("Study1_G1_T1_Synchronous.csv", pattern) %>% 
  as.data.frame() %>%
...
#Pattern to organize the str_match
pattern <- "Study(\\d+)_G(\\d+)_T(\\d+)_(\\w+)"

data_preprocess <- function(filelist, threshold){
  #Loading file 
  files = path_file(path = filelist)
  ...
  for (file in filelist) {
    data <- file %>% 
      ...
      dplyr::summarise(
        ...
        Resp1_c = mean(Resp1_c,na.rm=T),
        Resp2_c = mean(Resp2_c,na.rm=T)) %>% 
      mutate_if(is.numeric, scale) #should time be scaled?
    
    id_stuff <- str_match(data, pattern) %>% 
      as.data.frame() %>%
      rename("study" = "V2" , "group" = "V3", "trial" = "V4", "type" = "V5")
...
HR_data <- list.files(path = "\data", pattern = "Study1_G1", all.files = T, full.names = T) %>% 
    purrr::map_d(data_preprocess, 2.5)
...
read_HR_new <- function(filename, threshold) {
    
  #getting filenames and subsetting them
    filename <- path_file(path = filename) 
...
study4data <- list.files(path = './data', pattern = 'Study4', all.files = T, full.names = T) %>% 
    purrr::map_df(read_study4)
